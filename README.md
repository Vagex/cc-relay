...
- `Ollama (Relay on host)`: `http://127.0.0.1:11434/v1`, for running the relay directly on the same machine as Ollama.
- `Ollama (Relay in Docker, Ollama on Windows)`: `http://host.docker.internal:11434/v1`, for running the relay in Docker Desktop while Ollama runs on the Windows host.
